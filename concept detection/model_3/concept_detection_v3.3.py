# -*- coding: utf-8 -*-
"""ImageSemConcept.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Ne1KK3xKjYUMfAEQFeKnBLmCGf6eguJm
...
def load_images(pixel_x, pixel_y, img_names, folder_path):
  ...
  for img in img_names:
    ...
    # first resized using bilinear interpolation so that its smallest side has 310 pixels
    if image.shape[0] < image.shape[1]:
      scale_percent = 310 * 100 / image.shape[0]
    else :
      scale_percent = 310 * 100 / image.shape[1]
    image = cv2.resize(image, (int(image.shape[1] * scale_percent / 100), int(image.shape[0] * scale_percent / 100)), interpolation = cv2.INTER_LINEAR) #resize image
    
    cropped_image = tf.image.random_crop(image, size=[pixel_y, pixel_x])
    images_list.append(cropped_image)

  x_img = np.array(images_list)
  return x_img

"""create image train and valid sets"""

'''
x_train_img_array, x_train_img_array_names = load_images(299,299,path + 'train',train_start_id , train_end_id)
x_valid_img_array, x_valid_img_array_names = load_images(299,299,path + 'valid',valid_start_id , valid_end_id)
'''

'''
def show_image_from_jpg(image_name):
  image = tf.keras.preprocessing.image.load_img(image_name, target_size=(224, 224))
  plt.imshow(image)
'''

#show_image_from_jpg('/content/drive/MyDrive/ImageCLEF/train/ImageCLEFmedCaption_2022_train_000028.jpg')
...
for root, dirs, files in os.walk(train_path, topdown=False):
   for name in files:
      existing_train_img_names.append(os.path.splitext(name)[0])
'''
...
'''
existing_valid_img_names = []
for root, dirs, files in os.walk(valid_path, topdown=False):
   for name in files:
      existing_valid_img_names.append(os.path.splitext(name)[0])
'''
...
densenet201 pretrain
"""

'''
densenet201 = tf.keras.applications.DenseNet201()

x = densenet201.layers[-2].output
densenet201_prediction_layer = tf.keras.layers.Dense(len(my_MultiLabelBinarizer.classes_), activation= 'sigmoid')(x)
fine_tuned_densenet201 = tf.keras.models.Model(inputs = densenet201.input, outputs = densenet201_prediction_layer)
'''
...
with open( path + 'fateme/results/' +'valid_img_features.npy', 'wb') as f:
    np.save(f,valid_img_features,allow_pickle=True)

"""load predicted cui one hots from npy file"""

with open( path + 'fateme/results/' +'valid_img_features.npy', 'rb') as f:
...
valid_df_pred.to_csv(path + 'fateme/results/' + 'valid_concepts_results.csv', index=False, header=None)
...
with open( path + 'fateme/results/' +'test_img_features.npy', 'wb') as f:
    np.save(f,test_img_features,allow_pickle=True)

"""load predicted cui one hots from npy file"""

with open( path + 'fateme/results/' +'test_img_features.npy', 'rb') as f:
...
test_df_pred.to_csv(path + 'fateme/results/' + 'test_results.csv', index=False, header=None)
