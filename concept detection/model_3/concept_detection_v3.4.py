# -*- coding: utf-8 -*-
"""ImageSemConcept_v5.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1s6OjIKPRfusZj8BzgvXb9T8A2I0mY63h

data aug: normal, histogram_image, histogram_image_crop

w=310

test: histogram_image_crop
...
def load_images(pixel_x, pixel_y, img_names, folder_path , data_aug=False):
  ...
  for img in img_names:
    ...
    if(data_aug):
      ...
      #2 histogram_image
      histogram_image = cv2.equalizeHist(image) #apply Histograms Equalization
      images_list.append(cv2.resize(histogram_image, (pixel_y, pixel_x), interpolation = cv2.INTER_AREA))
  
      #3
      histogram_image_2 = cv2.resize(histogram_image ,(h, w), interpolation = cv2.INTER_LINEAR) #resize image
      histogram_image_2 = tf.image.random_crop(histogram_image_2, size=[pixel_y, pixel_x])
      images_list.append(histogram_image_2)
...
inceptionV3_prediction_layer = tf.keras.layers.Dense(len(my_MultiLabelBinarizer.classes_), activation=tf.keras.activations.sigmoid, kernel_regularizer='l2' )(x)
