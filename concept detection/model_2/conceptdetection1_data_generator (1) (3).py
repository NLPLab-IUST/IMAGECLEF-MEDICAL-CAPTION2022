...
train_df = pd.read_csv(path + 'ImageCLEFmedCaption_2022_concept_train.csv', delimiter="\t", index_col="ID").sample(train_limit).sort_values(by=['ID'])
print(f"2- read {len(train_df)} train images")

valid_df = pd.read_csv(path + 'ImageCLEFmedCaption_2022_concept_valid.csv', delimiter="\t", index_col="ID").sample(valid_limit).sort_values(by=['ID'])
...
import glob
test_img_names = [os.path.splitext(os.path.basename(x))[0] for x in glob.glob(test_path+'/*.jpg')]
...
def predict_and_save(valid_res, test_res, epoch):
  ...
  for th in threshold:
    ...
    classify(th_pred, test_img_names, f"e-{epoch}-threshold-{th}-classification.csv")
...
class SaveResultsCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
      ...
      self.model.save_weights(ysmn_path + f"/weights/e{epoch+1}_weights.h5", overwrite=True)
