...
def load_images_from_list(
    pixel_x, pixel_y, img_names, folder_path, apply_clahe=True, data_aug=True
):
    ...
    for img in img_names:
        ...
        if data_aug:
            ...
            hist_image = cv2.resize(
                hist_image, (pixel_y, pixel_x), interpolation=cv2.INTER_AREA
            )  # resize image
            images_list.append(hist_image)

            # 4
            hflip_image = cv2.flip(image, 1)
            hflip_image = cv2.resize(
                hflip_image, (pixel_y, pixel_x), interpolation=cv2.INTER_AREA
            )  # resize image
