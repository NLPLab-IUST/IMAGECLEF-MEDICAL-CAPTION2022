...
densenet_weights_path = ysmn_path + "/densenet121_weights_tf_dim_ordering_tf_kernels.h5"
...
concept_ids = pd.read_csv(path + 'IMAGECLEF_dataset/concepts.csv', sep="\t")["concept"].tolist()
print(f"1- read {len(concept_ids)} concepts")


train_df = pd.read_csv(path + 'IMAGECLEF_dataset/ImageCLEFmedCaption_2022_concept_train.csv', delimiter="\t", index_col="ID").sample(train_limit).sort_values(by=['ID'])
print(f"2- read {len(train_df)} train images")

valid_df = pd.read_csv(path + 'IMAGECLEF_dataset/ImageCLEFmedCaption_2022_concept_valid.csv', delimiter="\t", index_col="ID").sample(valid_limit).sort_values(by=['ID'])
...
class My_Custom_Generator(tf.keras.utils.Sequence) :
  ...
  def get_labels(self, img_names):
    ...
    return mlb.transform(self.concepts_df.loc[img_names]["cuis"].apply(lambda x: x.strip(";").split(";")).values)

    
    return batch_x, batch_y

gen_train = My_Custom_Generator(train_img_names, dataset_batchsize, folder_path=train_path, concepts_df=train_df)
gen_valid = My_Custom_Generator(valid_img_names, dataset_batchsize, folder_path=valid_path, concepts_df=valid_df)
...
fine_tuned_densenet = tf.keras.models.Model(inputs = densenet.input, outputs = prediction_layer)
...
fine_tuned_densenet.compile(loss = 'binary_crossentropy', optimizer= tf.keras.optimizers.Adam(learning_rate=initial_learning_rate), metrics = ['acc', f1_weighted])
...
import glob
test_img_names = [os.path.splitext(os.path.basename(x))[0] for x in glob.glob(test_path+'/*.jpg')][:test_limit]
...
def predict_and_save(valid_res, test_res, epoch):
  def evaluate(truth, th_pred):
    return f1_weighted(tf.convert_to_tensor(truth, "float32"), tf.convert_to_tensor(th_pred, "float32"))

  def classify(th_pred, img_names, filename):
      valid_cui_predictions = np.where(th_pred, mlb.classes_, "")#np.take(mlb.classes_, th_pred)
      # print(valid_cui_predictions.shape, th_pred.shape, re.sub(';+', ';', ";".join(valid_cui_predictions[0]))), 
      res=np.apply_along_axis(lambda x: re.sub(';+', ';',";".join(x)).strip(";"), 1, valid_cui_predictions)
      # print(res[0], res)
      valid_df_pred = pd.DataFrame(data={
          "ID": img_names,
          "cuis": res,
      })
      valid_df_pred.to_csv(ysmn_path + "/results/" + filename, index=False, header=None, sep="\t")

  threshold = [0.02,0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.19, 0.2, 0.21, 0.22, 0.23, 0.24, 0.25, 0.3, 0.4]
  valid_truth = mlb.transform(valid_df["cuis"].apply(lambda x: x.strip(";").split(";")).values)
  valid_f1s = []
  for th in threshold:
    ...
    classify(th_pred_test, test_img_names, f"e-{epoch}-threshold-{th}-classification.csv")



  pd.DataFrame({ "threshold": threshold, "f1": valid_f1s }).to_csv(ysmn_path + f"/results/valid_f1s_e{epoch}.csv", index=False)

class SaveResultsCallback(tf.keras.callbacks.Callback):
        
    def on_epoch_end(self, epoch, logs={}):
      if ((epoch+1) % 4 != 0 and (epoch+51) != 1):
        return
      ...
      self.model.save_weights(ysmn_path + f"/weights/e{epoch+51}_weights.h5", overwrite=True)
