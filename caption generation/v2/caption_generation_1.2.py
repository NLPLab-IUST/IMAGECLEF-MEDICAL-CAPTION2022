...
def load_images_from_list(
    ...
    for img in img_names:
        ...
        if data_aug:
            ...
            clahe_image = cv2.resize(
                clahe_image, (pixel_y, pixel_x), interpolation=cv2.INTER_AREA
            )  # resize image
...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        batch_x = load_images_from_list(
            224, 224, batch_names, self.folder_path, data_aug=self.is_train
        )
        batch_x = np.expand_dims(batch_x, axis=-1)
        batch_x = tf.image.grayscale_to_rgb(tf.convert_to_tensor(batch_x), name=None)
        batch_y = self.get_labels(batch_names)
        if self.is_train:
            y_rep = np.tile(batch_y, 2).reshape(batch_x.shape[0], -1)
...
class TestDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        batch_x = load_images_from_list(
            224, 224, batch_names, self.folder_path, data_aug=False
        )
        batch_x = np.expand_dims(batch_x, axis=-1)
        batch_x = tf.image.grayscale_to_rgb(tf.convert_to_tensor(batch_x), name=None)
        return batch_x
